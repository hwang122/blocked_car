...
/**
size_t chessboard::hashValue() const
{
    size_t hash_value = 0;
    for(int i = 0; i < height; i++)
    {
        for(int j = 0; j < width; j++)
        {
            hash_value += (i*width+j)*board[i][j];
        }
    }
    return hash_value;

    const int *begin = board[0];
    return boost::hash_range(begin, begin+sizeof(board));
}
**/
...
{
    template<>
    struct less<chessboard>
    {
        size_t operator()(const chessboard &c1, const chessboard &c2) const
        {
            int result = memcmp(c1.getBoard(), c2.getBoard(), sizeof(c1.getBoard()));
            return result != 0;
        }
    };
}
